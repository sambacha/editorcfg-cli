...
describe("eclint gulp plugin", () => {
	...
	describe("fix file", () => {
		...
		it("checks after fix", (done) => {
			...
			stream
				.pipe(eclint.check())
				.on("data", (file: eclint.IEditorConfigLintFile) => {
					expect(file.editorconfig).to.be.ok;
					expect(file.editorconfig.fixed).to.be.ok;
					expect(file.editorconfig.errors).to.have.lengthOf(0);
					done();
	...
	describe("check file", () => {
		...
		it("options.reporter", (done) => {
			const stream = eclint.check({
				reporter: (
					_file: eclint.IEditorConfigLintFile,
					error: Error
				) => {
					expect(error.message).to.have.equal(
						"invalid charset: utf-8-bom, expected: utf-8"
					);
					done();
				},
			});
	...
	describe("infer file", () => {
		...
		it("package.json (score)", (done) => {
			...
			stream.on("data", (file: File) => {
				const config = JSON.parse(String(file.contents));
				expect(config)
					.haveOwnProperty("trim_trailing_whitespace")
	...
	describe("end_of_line rule", () => {
		it('throws invalid "invalid newline: crlf, expected: lf"', (done) => {
			...
			stream.on("data", (file: eclint.IEditorConfigLintFile) => {
				...
				expect(error.name).to.equal("EditorConfigError");
				expect(error.message).to.equal(
					"invalid newline: crlf, expected: lf"
				);
	...
	describe("should plugin error", () => {
		...
		it("editorconfig.parse in eclint.check", (done) => {
			stub = sinon
				.stub(editorconfig, "parse")
				.rejects(new Error("check editorconfig testcase"));
			const stream = eclint.check();

			stream.on("error", (error: PluginError) => {
				expect(error)
					.haveOwnProperty("plugin")
					.and.to.be.equal("ECLint");
				expect(error)
					.haveOwnProperty("message")
					.and.to.be.equal("check editorconfig testcase");
				done();
			});
		...
		it("charset.check", (done) => {
			stub = sinon
				.stub(charset, "check")
				.throws(new Error("check testcase"));
			const stream = eclint.check();
			...
			stream.on("error", (error: PluginError) => {
				...
				expect(error)
					.haveOwnProperty("message")
					.and.to.be.equal("check testcase");
				done();
			});
		...
		it("editorconfig.parse in eclint.fix", (done) => {
			stub = sinon
				.stub(editorconfig, "parse")
				.rejects(new Error("fix editorconfig testcase"));
			const stream = eclint.fix();
			...
			stream.on("error", (error: PluginError) => {
				...
				expect(error)
					.haveOwnProperty("message")
					.and.to.be.equal("fix editorconfig testcase");
				done();
			});
		...
		it("charset.infer", (done) => {
			stub = sinon
				.stub(charset, "infer")
				.throws(new Error("infer testcase"));
			const stream = eclint.infer();
			...
			stream.on("error", (error: PluginError) => {
				...
				expect(error)
					.haveOwnProperty("message")
					.and.to.be.equal("infer testcase");
				done();
			});
