...
function resolve(settings: eclint.ISettings): number {
    const result = settings.indent_size === 'tab' ? settings.tab_width : settings.indent_size
...
function checkLine(line: doc.Line, indentSize: number): EditorConfigError {
    if (
        ...
        (line.blockCommentStart && line.prefix.length === line.blockCommentStart.prefix.length + line.padSize)
    ) {
        return
    }
