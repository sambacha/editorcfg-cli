...
describe("eclint doc", () => {
	it("block comment rule", () => {
		...
		expect(lines[0]).haveOwnProperty("isBlockCommentStart").and.to.be.not
			.ok;
		...
		expect(lines[2]).haveOwnProperty("isBlockCommentStart").and.to.be.not
			.ok;
		...
		expect(lines[3]).haveOwnProperty("isBlockCommentStart").and.to.be.not
			.ok;
		...
		expect(lines[4]).haveOwnProperty("isBlockCommentStart").and.to.be.not
			.ok;
	...
	it("config without block_comment", () => {
		const lines = doc.create(
			["<!--[if IE]>", "You are useing IE.", "<![endif]-->"].join("\n"),
			{
